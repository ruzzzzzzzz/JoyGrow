/**
 * Advanced Document Parser - OFFLINE-FRIENDLY VERSION
...
async function parsePDFDocument(file: File): Promise<ParsedDocument> {
  try {
    ...
    if (fullText.length < 50) {
      throw new Error(
        'Could not extract enough readable text from PDF. The file may be image-based or corrupted.'
      );
    }
...
async function parseDOCXDocument(file: File): Promise<ParsedDocument> {
  try {
    ...
    if (finalText.length < 50) {
      throw new Error(
        'Could not extract readable text from DOCX file. The document may be empty or corrupted.'
      );
    }
    ...
    throw new Error(
      'Failed to parse DOCX file. Please ensure the file is a valid Word document or try converting to TXT format.'
    );
  }
}
...
async function parseTXTDocument(file: File): Promise<ParsedDocument> {
  try {
    ...
    if (trimmedText.length < 50) {
      throw new Error(
        'Text file is too short or empty. Please provide a document with at least 50 characters.'
      );
    }
  ...
  Object.entries(topicKeywords).forEach(([topic, keywords]) => {
    const matchCount = keywords.filter(
      keyword =>
        ...
        lowerTerms.some(term => term.includes(keyword))
    ).length;
